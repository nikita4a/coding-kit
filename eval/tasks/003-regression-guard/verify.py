...
def main() -> int:
    ...
    #    against the pristine fixture, and (c) fail when only `clamp` is
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if r_pris.returncode != 1:
            print(f"FAIL: candidate tests did not fail on pristine calc.py (rc={r_pris.returncode}); missing regression test for the clamp defect:\n{r_pris.stdout}\n{r_pris.stderr}")
            return 1
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if r_rev.returncode != 1:
            print(f"FAIL: candidate tests did not fail when clamp was reverted to pristine (rc={r_rev.returncode}); regression test does not target clamp:\n{r_rev.stdout}\n{r_rev.stderr}")
            return 1
